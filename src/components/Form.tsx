  ...
  return (
    <form className="flex flex-col gap-3" onSubmit={handleSubmit}>
      ...
      <div className="flex flex-col">
        ...
        <input type="email" placeholder="Insira seu melhor e-mail" className="rounded-lg py-2 px-2 text-sm placeholder:text-sm placeholder:text-stone-400 value={name} " value={email} onChange={(e) => setEmail(e.target.value)}/>
      </div>
      {errors?.email && 
      ...
      <button type="submit" className="bg-slate-600 hover:bg-slate-500 font-medium text-sm py-2 px-4 rounded-lg text-white class">Cadastrar</button>
    </form>
  )
}
